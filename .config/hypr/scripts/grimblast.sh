...
takeScreenshot() {
    ...
    GEOM=$2
    OUTPUT=$3
    c_state=$(hyprctl --batch -j \
        "getoption cursor:no_hardware_cursors;\
        getoption cursor:inactive_timeout" |
        jq -r '[.int, .float] | map(select(. != null)) | @csv')
    local hw_cursor=${c_state%%$'\n'*}
    local t_out=${c_state#*$'\n'}
    if [ -z "${CURSOR}" ] || [[ "${hw_cursor}" != 0 ]]; then
        hyprctl --batch -q \
            "keyword cursor:no_hardware_cursors false;\
             keyword cursor:inactive_timeout 0.000001"
        sleep 0.2
    fi
    if [ -n "$OUTPUT" ]; then
    ...
    hyprctl --batch -q \
        "keyword cursor:no_hardware_cursors ${hw_cursor};\
        keyword cursor:inactive_timeout ${t_out}"
}
