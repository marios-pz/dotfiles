...
audio_icon=$(amixer -M get Master | awk -F'[][]' '/Mono/{printf "%d 🔉", $2; if ($4 == "off") {printf " 🔇"};}')


audiovolume=$(amixer -M get Master | awk -F'[][]' '/Front Left/ { print $2 }' | sed 's/%//')
...
# Separators: \| ❘ ❙ ❚
# Misc: 🐧 💎 💻 💡 ⭐ 📁 ↑ ↓ ✉ ✅ ❎
echo $audiovolume $audio_icon $battery_info 🔋 $date_formatted
